...
class TransientView: UIView {
    public var tool:PTPenTool?
    ...
    override func draw(_ rect: CGRect) {
        ...
        guard let shape = tool?.shapeInProgress else {return }
...
public class DrawsanaView: UIView {
  ...
  public private(set) var tool: PTPenTool?
  ...
  // MARK: Views
    ...
    private let transientContentView = TransientView()
  ...
  public var lastRender = clock()
  public var renderInProgress = false
  public var isShapeInProgress = false
  public var shapeInProgress:PTPenShape?
    public var lastRenderTime = 0.0
  // MARK: Init
  ...
  private func commonInit() {
    ...
    transientContentView.backgroundColor = .clear
    
    addSubview(drawingContentView)
    addSubview(transientContentView)
    ...
    transientContentView.translatesAutoresizingMaskIntoConstraints = false
    NSLayoutConstraint.activate([
      transientContentView.leftAnchor.constraint(equalTo: leftAnchor),
      transientContentView.rightAnchor.constraint(equalTo: rightAnchor),
      transientContentView.topAnchor.constraint(equalTo: topAnchor),
      transientContentView.bottomAnchor.constraint(equalTo: bottomAnchor),
    ])
    ...
    drawingContentView.layer.backgroundColor = UIColor.white.cgColor
  ...
  public func set(tool: PTPenTool, shape: Shape? = nil) {
...
//        let begin = clock()
        ...
        //NSLog("DrawsanaView.transientBufferWithShapeInProgress rendering transientBuffer: took %0.3f seconds",Double(clock() - begin) / Double(CLOCKS_PER_SEC));
//        let begin2 = clock()
        self.tool?.renderShapeInProgress(transientContext: $0)
       // NSLog("DrawsanaView.tool.renderShapeInProgress: took %0.3f seconds",Double(clock() - begin2) / Double(CLOCKS_PER_SEC));
        
      }
      self.drawingContentView.layer.drawsAsynchronously = true;
        if self.isShapeInProgress {
      ...
      if self.tool?.isProgressive == true {
        ...
        self.lastRenderTime =  Double(clock() - renderStart) / Double(CLOCKS_PER_SEC)
        self.lastRender = renderStart
    }
...
//      }
    ...
    case .changed:
      ...
      let timeSinceLastRender = Double(clock() - self.lastRender)/Double(CLOCKS_PER_SEC)
      print("Time since last render:",timeSinceLastRender," lastRenderTime: ",lastRenderTime," Render in progress: ",renderInProgress)
//      if timeSinceLastRender >= lastRenderTime && !renderInProgress{
//        print("Starting render at: ",Double(clock())/Double(CLOCKS_PER_SEC))
//        DispatchQueue.main.async {
//            self.renderInProgress = true
//            updateUncommittedShapeBuffers()
//            self.renderInProgress = false
//        }
//      }
      let lp = lastPoint ?? .zero
//      self.transientContentView.setNeedsDisplay(CGRect(x: CGFloat.minimum(point.x, lp.x), y: CGFloat.minimum(point.y, lp.y), width: (CGFloat.maximum(point.x, lp.x) - CGFloat.minimum(point.x, lp.x) ), height: (CGFloat.maximum(point.y, lp.y) - CGFloat.minimum(point.y, lp.y) )))
        self.transientContentView.setNeedsDisplay()


    case .ended:
      isShapeInProgress = false
  ...
  private func reapplyLayerContents() {
    self.transientContentView.tool = nil
